defmodule Mix.Tasks.UnusedFunctionsTest do
  ...
  describe "run/1" do
    test "runs with default options" do
      ...
      output = capture_mix_output(fn -> Mix.Tasks.UnusedFunctions.run([]) end)
      assert output =~ "Unused Public Functions" or output =~ "No unused public functions"
    end

    test "runs with --format json" do
      output = capture_mix_output(fn -> Mix.Tasks.UnusedFunctions.run(["--format", "json"]) end)
      assert {:ok, _} = Jason.decode(output)
    end

    test "runs with --format github" do
      output = capture_mix_output(fn -> Mix.Tasks.UnusedFunctions.run(["--format", "github"]) end)
      # GitHub format is either empty or ::warning lines
      assert output == "" or output =~ "::warning"
    end

    test "runs with --include-deps" do
      output = capture_mix_output(fn -> Mix.Tasks.UnusedFunctions.run(["--include-deps"]) end)
